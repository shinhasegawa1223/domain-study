...
4. [リクエストからSQLまでの流れ](#-リクエストからsqlまでの流れ)
5. [Before/After 比較](#beforeafter-比較)
6. [戦術的設計の構成要素](#戦術的設計の構成要素)
7. [まとめ](#まとめ)
...
エンドポイントが叩かれてから、SQLが実行されるまでの処理の流れを追います。
...
│                                                                       │
│  ① クライアント                                                      │
│     POST /orders { customerId: "123", items: [...] }                 │
│                            ↓                                          │
├──────────────────────────────────────────────────────────────────────┤
│  ② プレゼンテーション層（Controller）                                 │
│     リクエストを受け取り、アプリケーション層を呼び出す                  │
│                            ↓                                          │
├──────────────────────────────────────────────────────────────────────┤
│  ③ アプリケーション層（UseCase）                                      │
│     ユースケースを調整。リポジトリを呼び出す                           │
│                            ↓                                          │
├──────────────────────────────────────────────────────────────────────┤
│  ④ ドメイン層（Entity / Value Object）                               │
│     ビジネスロジックを実行                                            │
│                            ↓                                          │
├──────────────────────────────────────────────────────────────────────┤
│  ⑤ インフラ層（Repository実装）                                      │
...
class CreateOrderUseCase {
  constructor(
    private customerRepo: CustomerRepository,  // インターフェースに依存
...
class Order {
  ...
  // ④ ファクトリメソッドでビジネスルールを適用
  static create(customerId: CustomerId, itemInputs: OrderItemInput[]): Order {
    // ビジネスルール：注文には1つ以上の商品が必要
    ...
    for (const item of order.items) {
      await db.query(
        `INSERT INTO order_items (order_id, product_id, quantity)
         VALUES ($1, $2, $3)`,
        [order.id.value, item.productId.value, item.quantity]
      );
    }
  }
}
...
| ステップ | 層 | ファイル | 処理内容 |
|:-------:|:--:|---------|---------|
| ① | - | (クライアント) | `POST /orders` リクエスト送信 |
| ② | Presentation | `order-controller.ts` | リクエスト受信、UseCase呼び出し |
| ③ | 🎯 Application | `create-order-usecase.ts` | ユースケース調整、リポジトリ呼び出し |
| ④ | 📦 Domain | `order.ts` | ビジネスロジック実行 |
| ⑤ | 🗄️ Infrastructure | `postgres-order-repository.ts` | SQL実行 |
| ⑥ | - | (PostgreSQL) | データ永続化 |
...
| 観点 | 説明 |
|------|------|
| **SQLを書くのはインフラ層だけ** | ③④はSQLを知らない |
| **アプリケーション層はリポジトリを呼ぶ** | インターフェース経由でアクセス |
| **ドメイン層はビジネスロジックに集中** | 永続化を意識しない |
